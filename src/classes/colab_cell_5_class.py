...
class FixedFootballAnalysis:
    ...
    def process_detections(self, results, frame_num):
        """Process YOLO detections with better filtering"""
        detections = []
        ...
        for box in results.boxes:
            ...
            # Lower confidence threshold and filter for person class (0)
            if conf > 0.3 and cls == 0:  # Person class
                ...
                if 1000 < area < 50000:  # Reasonable person size
                    detections.append({
                        'frame': frame_num,
    ...
    def detect_passes(self, players, frame_num):
        """Detect passes between players with improved logic"""
        passes = []
        
        if len(players) < 2:
            return passes
        
        # Only check every 5th frame to avoid too many detections
        if frame_num % 5 != 0:
            return passes
        ...
        for player1_id, player1 in players.items():
            for player2_id, player2 in players.items():
                ...
                # Improved pass detection criteria
                if 30 < distance < 300:  # More reasonable pass distance
                    # Check if players are moving towards each other
                    if self.is_moving_towards(player1, player2):
                        pass_type = 'long' if distance > 150 else 'short'
                        
                        # Improved success/failure simulation
                        success_prob = 0.9 if distance < 100 else 0.7 if distance < 200 else 0.5
                        ...
                        passes.append({
                            ...
                            'type': pass_type,
                            'success': success,
                            'confidence': (player1['confidence'] + player2['confidence']) / 2
                        })
